...
class CommandInterface(Cmd):
    ...
    def do_info(self, obj):
        ...
        if not obj:
            print("error: no info object specified, expected 'calls'" +
                  " or 'obs'")
        elif obj == 'calls':
            data = build_json('info', 'calls')
            client.transport.write(data.encode())
        elif obj == 'ops':
            data = build_json('info', 'ops')
            client.transport.write(data.encode())
